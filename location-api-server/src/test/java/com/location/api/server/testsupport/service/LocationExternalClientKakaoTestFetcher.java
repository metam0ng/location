package com.location.api.server.testsupport.service;

import com.location.external.client.spec.LocationExternalClientFetcher;
import com.location.external.client.spec.code.ExternalType;
import com.location.external.client.spec.dto.Coordinate;
import com.location.external.client.spec.dto.LocationInformation;
import com.location.external.client.spec.dto.LocationInformations;

import java.util.List;

public class LocationExternalClientKakaoTestFetcher implements LocationExternalClientFetcher {
    @Override
    public boolean isSupport(ExternalType externalType) {
        return ExternalType.KAKAO == externalType;
    }

    @Override
    public LocationInformations searchLocationByKeyword(String keyword,
                                                        int pageSize,
                                                        int totalSize) {
        if(keyword.equals("카카오")){
            return LocationInformations.from(List.of(
                    LocationInformation.builder()
                            .name("카카오")
                            .coordinate(Coordinate.of(127.1104335101161, 37.39570088983171))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오 스페이스닷원")
                            .coordinate(Coordinate.of(126.57066130083415, 33.450682729588145))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오 스페이스닷투")
                            .coordinate(Coordinate.of(126.570875463183, 33.4526219140826))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오프렌즈 판교아지트점")
                            .coordinate(Coordinate.of(127.1100869772751, 37.39581744474611))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오판교아지트")
                            .coordinate(Coordinate.of(127.11036420512991, 37.39541713271851))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오뱅크")
                            .coordinate(Coordinate.of(127.112119228848, 37.395455126629855))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오모빌리티")
                            .coordinate(Coordinate.of(127.1101250888609, 37.39407843730005))
                            .build(),
                    LocationInformation.builder()
                            .name("판교 H스퀘어 (카카오 판교사옥) 전기차충전소 (테슬라전용)")
                            .coordinate(Coordinate.of(127.10863811607922, 37.4021232296443))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오엔터테인먼트 판교오피스(스토리부문)")
                            .coordinate(Coordinate.of(127.108996408808, 37.4007470412071))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오브이엑스")
                            .coordinate(Coordinate.of(127.10878160491359, 37.401377953592146))
                            .build()
            ));
        } else if (keyword.equals("네이버")) {
            return LocationInformations.from(List.of(
                    LocationInformation.builder()
                            .name("네이버 바이브")
                            .coordinate(Coordinate.of(129.57066130083415, 33.450682729588145))
                            .build(),
                    LocationInformation.builder()
                            .name("네이버 포인트")
                            .coordinate(Coordinate.of(129.57066130083415, 33.450682729588145))
                            .build(),
                    LocationInformation.builder()
                            .name("네이버 판교아지트")
                            .coordinate(Coordinate.of(129.57066130083415, 33.450682729588145))
                            .build(),
                    LocationInformation.builder()
                            .name("네이버 쇼핑")
                            .coordinate(Coordinate.of(127.1100869772751, 37.39581744474611))
                            .build(),
                    LocationInformation.builder()
                            .name("네이버 뉴스")
                            .coordinate(Coordinate.of(127.11036420512991, 37.39541713271851))
                            .build(),
                    LocationInformation.builder()
                            .name("네이버 예약")
                            .coordinate(Coordinate.of(127.112119228848, 37.395455126629855))
                            .build(),
                    LocationInformation.builder()
                            .name("네이버 웹툰")
                            .coordinate(Coordinate.of(127.1101250888609, 37.39407843730005))
                            .build(),
                    LocationInformation.builder()
                            .name("네이버 그린팩토리")
                            .coordinate(Coordinate.of(127.10863811607922, 37.4021232296443))
                            .build(),
                    LocationInformation.builder()
                            .name("네이버 메일")
                            .coordinate(Coordinate.of(127.108996408808, 37.4007470412071))
                            .build(),
                    LocationInformation.builder()
                            .name("네이버 블로그")
                            .coordinate(Coordinate.of(127.10878160491359, 37.401377953592146))
                            .build()
            ));

        } else if (keyword.equals("카카오 프렌즈")) {
            return LocationInformations.from(List.of(
                    LocationInformation.builder()
                            .name("카카오 프렌즈")
                            .coordinate(Coordinate.of(127.1104335101161, 37.39570088983171))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오 프렌즈 스페이스닷원")
                            .coordinate(Coordinate.of(126.57066130083415, 33.450682729588145))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오 프렌즈 스페이스닷투")
                            .coordinate(Coordinate.of(126.570875463183, 33.4526219140826))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오프렌즈 프렌즈 판교아지트점")
                            .coordinate(Coordinate.of(127.1100869772751, 37.39581744474611))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오판교아지트")
                            .coordinate(Coordinate.of(127.11036420512991, 37.39541713271851))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오뱅크")
                            .coordinate(Coordinate.of(127.112119228848, 37.395455126629855))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오모빌리티")
                            .coordinate(Coordinate.of(127.1101250888609, 37.39407843730005))
                            .build(),
                    LocationInformation.builder()
                            .name("판교 H스퀘어 (카카오 판교사옥) 전기차충전소 (테슬라전용)")
                            .coordinate(Coordinate.of(127.10863811607922, 37.4021232296443))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오엔터테인먼트 판교오피스(스토리부문)")
                            .coordinate(Coordinate.of(127.108996408808, 37.4007470412071))
                            .build(),
                    LocationInformation.builder()
                            .name("카카오브이엑스")
                            .coordinate(Coordinate.of(127.10878160491359, 37.401377953592146))
                            .build()
            ));
        } else {
            return null;
        }

    }
}
